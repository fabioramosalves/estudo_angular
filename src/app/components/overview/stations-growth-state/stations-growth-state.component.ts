...
@Component({
  selector: 'app-stations-growth-state',
  templateUrl: './stations-growth-state.component.html',
  styleUrl: './stations-growth-state.component.css',
  standalone: true,
  imports: [
    CommonModule,
    MatCardModule,
...
export class StationsGrowthStateComponent implements OnInit {
  ...
  updateCharts(): void {
    this.getRegionGrowth('state');
  }

  getRegionGrowth(region: 'city' | 'state'): void {
    this.DashboardService.getRegionGrowth(region, this.selectedDate).subscribe((data: RegionGrowthResponse[]) => {
